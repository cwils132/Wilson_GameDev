...
import com.wilson.gdx.game.Emerald;
...
/**
 * This class is to handle all collisions for Box2D. We only need to worry about
 * Rocks' and the Player at the moment so that is all it will be checking
 * against.
 * 
 * @author Chris
 *
...
public class CollisionHandler implements ContactListener
{
	...
	/**
	 * Adds listeners to check if contact occurs between the Player and another
	 * object.
	 * 
	 * @param categoryA
	 * @param categoryB
	...
	public void endContact(Contact contact)
	{
		...
		// Gdx.app.log("CollisionHandler-end A",
		// fixtureA.getBody().getLinearVelocity().x+" :
		// "+fixtureA.getBody().getLinearVelocity().y);
		// Gdx.app.log("CollisionHandler-end B",
		// fixtureB.getBody().getLinearVelocity().x+" :
		// "+fixtureB.getBody().getLinearVelocity().y);
		ContactListener listener = getListener(fixtureA.getFilterData().categoryBits,
		        fixtureB.getFilterData().categoryBits);
		if (listener != null)
		{
	...
	/**
	 * Handles all collisions and and boolean values
	 * related to movement in the game world. Also plays sound effects
	 * if applicable.
	 * 
	 * Instructs items when they have been collected and flags them for
	 * removal.
	 * 
	 * @param playerFixture
	 * @param objFixture
	...
	private void processPlayerContact(Fixture playerFixture, Fixture objFixture)
	{
		...
		else if (objFixture.getBody().getUserData() instanceof RoughRock)
		{
			world.resetJump();
			world.level.bunnyHead.grounded = true;
			world.level.bunnyHead.jumping = false;
		...
		else if (objFixture.getBody().getUserData() instanceof Feather)
		{
			Feather feather = (Feather) objFixture.getBody().getUserData();
			if (!feather.collected)
			{
				AudioManager.instance.play(Assets.instance.sounds.pickupFeather);
				world.level.bunnyHead.setFeatherPowerup(true);
			}

			feather.collected = true;
			world.score += feather.getScore();
			world.flagForRemoval(feather);
		}
		else if (objFixture.getBody().getUserData() instanceof Emerald)
		{
			Emerald emerald = (Emerald) objFixture.getBody().getUserData();
			if (!emerald.collected)
			{
				AudioManager.instance.play(Assets.instance.sounds.pickupFeather);
				world.level.bunnyHead.setEmeraldPowerup(true);
			}

			emerald.collected = true;
			world.score += emerald.getScore();
			world.flagForRemoval(emerald);
		}
	}
}